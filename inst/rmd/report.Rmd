...
my.theme <- ggplot2::theme_light() +
  ggplot2::theme(
    ...
    strip.text = ggplot2::element_text(size = 16, colour = "black")
  )
```
...
unique.study.samples <- length(which(unique(res.dups[, "Sample.ID"])))
res.dups <- res.dups[duplicated(res.dups[, "Sample ID"]) |
  duplicated(res.dups[, "Sample ID"], fromLast = TRUE), ]
n.tech.reps <- length(which(unique(res.dups[, "Sample ID"])))
## compute ICC to match existing method
ICC.est <- ICC::ICCest(
  factor(tech.reps[, "Sample ID"]),
  tech.reps[, "Standardized T/S Ratio"]
)
```
...
res.int <- res[res[, "Sample ID"] == "NA07057"]
mean.cv <- mean(res.int[, "Telo %CV"], na.rm = TRUE)
box.data <- data.frame(
  x = rep(res.int[, "Sample ID"], 2),
  y = c(
...
cv.box <- cv.box + ggplot2::geom_boxplot()
...
and its 95% confidence interval were `r ICC.est$ICC` 
(`r ICC.est$LowerCI`,`r ICC.est$UpperCI`).
