...
#' @param source.plate.contents.search.path Character vector with path to test case Data/Analysis *.xlsx files
...
read.export.datum <- function(exp.control.filenames,
                              source.plate.contents.search.path = NA,
    ...
    stopifnot(is.vector(source.plate.contents.search.path, mode = "character") | isTRUE(is.na(source.plate.contents.search.path)))
    stopifnot(is.logical(subject.list.from.input.path) | isTRUE(is.na(subject.list.from.input.path)))
    stopifnot(is.vector(plate.content.report, mode = "character") | isTRUE(is.na(plate.content.report)))
    stopifnot(is.vector(plate.list, mode = "character") | isTRUE(is.na(plate.list)))
    ## read exported data
    ...
    ## if the caller requests subject list information pulled from an existing Data/Analysis/*xlsx file
    if (isTRUE(!is.na(subject.list.from.input.path)) & subject.list.from.input.path & isTRUE(!is.na(source.plate.contents.search.path))) {
    ...
    } else if (isTRUE(!is.na(plate.content.report)) & isTRUE(!is.na(plate.list))) {
        ## the caller provides enough information to pull data from a plate content report and list
        ...
        ## pending further example datasets, the current link is: content file column "Description" will have
        ## an entry "{Project.Code}_Intermediate_{Analysis.Code}". That row's corresponding "Identifier" entry
        ## links to the plate content file column "Plate.ID", and then you want the content file entries
        ...
        matched.index <- which(grepl(paste("^.*_Intermediate_", obj@Analysis.Code, "$", sep = ""), plate.list.data$Description))
        stopifnot(length(matched.index) == 1)
        stopifnot(length(which(plate.content.data$Plate.ID == plate.list.data$Identifier[matched.index])) > 0)
        target.contents <- plate.content.data[plate.content.data$Plate.ID == plate.list.data$Identifier[matched.index],]
        target.contents$Well.ID <- as.vector(target.contents$Well.ID, mode = "character")
        ...
        stop("In read.export.datum(), no valid method for pulling source plate contents detected")
    }
